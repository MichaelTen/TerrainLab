...
Creates a structurally valid, non-proprietary minimal set of Ultima Online style .mul files
that CentrED# and ClassicUO-based tooling commonly expect to exist.

Goal
- Run CentrED# client and server with a legally clean, blank asset set
- Later fill everything with 100% original art and metadata (via UO Fiddler or your own tools)
- Optionally consume or convert these formats into your own Godot-native formats

Creates (in output directory):
  - art.mul            (empty container; no artwork)
  - artidx.mul         (index records pointing to "no art")
  ...
  - texmaps.mul        (empty texture container; no textures)
  - texidx.mul         (index records pointing to "no texture")
  - light.mul          (empty light patterns container)
  - lightidx.mul       (index records pointing to "no light pattern")
  - multi.mul          (blank multi container; 1 byte placeholder to avoid zero-length loader bugs)
  - multi.idx          (index records pointing to "no multi")
...
Usage examples:
  ...
  python AxonometricLab.py --out ClientMin --force --facet 0 --map-width 256 --map-height 256 --default-land 2 --default-z 0
"""
...
def write_uo_index_3dwords(path: Path, entries: int, force: bool, missing_offset: int = 0xFFFFFFFF) -> None:
    """
    Common MUL index layout: 12 bytes per entry
      uint32 offset, uint32 length, uint32 extra
    Missing entry convention: offset = 0xFFFFFFFF, length = 0, extra = 0
    Used by: artidx.mul, texidx.mul, multi.idx, and many other UO idx files.
    """
    rec = struct.pack("<III", missing_offset & 0xFFFFFFFF, 0, 0)
    write_file(path, rec * entries, force)
...
def make_art_mul(out_dir: Path, force: bool) -> None:
    # art.mul has no header. Empty is acceptable when artidx marks "no art".
    write_file(out_dir / "art.mul", b"", force)

def make_artidx_mul(out_dir: Path, force: bool, idx_entries: int) -> None:
    write_uo_index_3dwords(out_dir / "artidx.mul", idx_entries, force)
...
def make_hues_mul(out_dir: Path, force: bool, hue_groups: int) -> None:
    # Each group: uint32 header + 8 hue entries
    # Hue entry: 32*uint16 colors (64) + uint16 start + uint16 end + char[20] name = 88 bytes
    def writer(f):
        hue_entry = (
            (b"\x00" * (32 * 2)) +          # 32 uint16 colors
...
def make_texmaps_mul(out_dir: Path, force: bool) -> None:
    # Empty container is fine if texidx marks every entry as missing.
    write_file(out_dir / "texmaps.mul", b"", force)

def make_texidx_mul(out_dir: Path, force: bool, idx_entries: int) -> None:
    write_uo_index_3dwords(out_dir / "texidx.mul", idx_entries, force)
...
def make_light_mul(out_dir: Path, force: bool) -> None:
    # Empty container is acceptable if lightidx marks all patterns as missing.
    write_file(out_dir / "light.mul", b"", force)

def make_lightidx_mul(out_dir: Path, force: bool, idx_entries: int) -> None:
    """
    lightidx.mul is commonly 12 bytes per entry, but not 3 dwords.
    Typical layout:
      uint32 offset, uint32 length, uint16 width, uint16 height
    Missing entry convention:
      offset = 0xFFFFFFFF, length = 0, width = 0, height = 0
    """
    rec = struct.pack("<IIHH", 0xFFFFFFFF, 0, 0, 0)
    write_file(out_dir / "lightidx.mul", rec * idx_entries, force)
...
def make_multi_mul(out_dir: Path, force: bool) -> None:
    """
    Important:
    Some ClassicUO-based loaders do not handle a zero-length multi.mul gracefully
    (they end up with a null reader and later Seek causes NullReferenceException).
    We write a 1-byte placeholder (0x00) to keep the container legally clean while
    avoiding a zero-length stream edge case.
    """
    write_file(out_dir / "multi.mul", b"\x00", force)

def make_multi_idx(out_dir: Path, force: bool, idx_entries: int) -> None:
    # multi.idx uses the common 12-byte 3-dword index layout.
    write_uo_index_3dwords(out_dir / "multi.idx", idx_entries, force)
...
def make_map_and_statics(
    ...
    # Classic map block:
    #   int32 header
    #   64 tiles, each tile: uint16 landTileId + int8 z
    ...
    # Staidx entry:
    #   int32 lookup, int32 length, int32 extra
    # Empty statics: lookup = -1, length = 0, extra = 0
...
def main() -> int:
    p = argparse.ArgumentParser(
        description="Generate minimal, structurally valid UO-style .mul files (no proprietary content)."
    )
    ...
    p.add_argument("--hue-groups", type=int, default=375,
                   help="Number of hue groups (default 375 -> 3000 hues).")

    # Textures: many toolchains assume 0x4000 texture slots.
    p.add_argument("--texidx-entries", type=lambda x: int(x, 0), default=0x4000,
                   help="Number of texidx records (default 0x4000). Accepts hex like 0x4000.")

    # Lights: classic clients often have around 100 light patterns.
    p.add_argument("--lightidx-entries", type=int, default=100,
                   help="Number of lightidx records (default 100).")

    # Multis: keep it modest but configurable. 0x4000 is a common comfortable size for blank placeholders.
    p.add_argument("--multiidx-entries", type=lambda x: int(x, 0), default=0x4000,
                   help="Number of multi.idx records (default 0x4000). Accepts hex like 0x4000.")
    ...
    make_texmaps_mul(out_dir, args.force)
    make_texidx_mul(out_dir, args.force, idx_entries=args.texidx_entries)

    make_light_mul(out_dir, args.force)
    make_lightidx_mul(out_dir, args.force, idx_entries=args.lightidx_entries)

    make_multi_mul(out_dir, args.force)
    make_multi_idx(out_dir, args.force, idx_entries=args.multiidx_entries)
    ...
    print("  texmaps.mul      =", sz("texmaps.mul"), "bytes")
    print("  texidx.mul       =", sz("texidx.mul"), "bytes")
    print("  light.mul        =", sz("light.mul"), "bytes")
    print("  lightidx.mul     =", sz("lightidx.mul"), "bytes")
    print("  multi.mul        =", sz("multi.mul"), "bytes")
    print("  multi.idx        =", sz("multi.idx"), "bytes")
    ...
    print("Index info:")
    print(f"  artidx entries    = {args.artidx_entries} (all empty, art.mul is blank)")
    print(f"  texidx entries    = {args.texidx_entries} (all empty, texmaps.mul is blank)")
    print(f"  lightidx entries  = {args.lightidx_entries} (all empty, light.mul is blank)")
    print(f"  multi.idx entries = {args.multiidx_entries} (all empty, multi.mul has 1-byte placeholder)")
